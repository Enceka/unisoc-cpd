...
│                              band, nr, ims, sms, ussd, call, data, nv, diag
...
└── docs/BASEBAND-CONTRACTS.md     W0: the channel/command/timing contract
```
...
cargo test                     # 66 tests, no device needed
```
...
daemon and against ours and the two summaries diffed.  `link` is native-only:
there is no vendor counterpart to compare a channel-ownership measurement with.
...
| | |
|---|---|
| core (channel/at/telemetry/profile/CLI) | implemented, 66 tests green |
| capabilities | `link`, `sim`, `register`, `signal`, `operator`, `cfun`, `band`, `nr`, `ims`, `sms`, `ussd`, `call`, `data`, `nv`, `diag` |
...
| on-device, AT | **nothing has been soaked yet.** No profile is `verified`; the daemon has not been the owner of `/dev/stty_nr1` |
